...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=556193845817:+55 619384-5817\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve @${num.split('@')[0]}\Seja bem vindo ao grupo *${mdata.subject}* comporte-se para nao levar ban`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ Calma ae ta  processando',
				success: '️❬ ✔ ❭ prontinho',
				error: {
					stick: 'Desculpe,eu falhei:(',
					Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '❬❗❭ COMANDO SÓ PODE SER USADO EM GRUPO',
					ownerG: '❬❗❭ COMANDO EXCLUSIVO PRO GUSTAKK',
					ownerB: '❬❗❭  OCMANDO EXCLUSIVO PRO GUSTAKK',
					admin: '❬❗❭ COMANDO  EXCLUSIVO PARA ADM',
          ...
          me = client.user
					...
					break
				case 'blocklist': 
          teks = 'bloqueado com sucesso:\n'
					for (let block of blocked) {
						teks += `┣➢ @${block.split('@')[0]}\n`
					}
					teks+=`Total:${blocked.length}`
					client.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mencionado": blocked}})
					break
                case 'hidetag':
                ...
                case 'glitch':
					...
					break
                case 'wolflogo':
                      if (args.length < 1) return reply(`Cade o texto po?`)
                      gh = body.slice(10)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
                      reply(mess.wait)
			    ...
			    case 'randomhentai':
				    try {
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'gosta de um hentai ne '})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
                ...
                if (!isNsfw) return reply('NSFW não está ativo')
                   ...
                   if (anu.error) return reply(anu.error)
                   hasil = `*título* \n${anu.result.judul} *genero* \n${anu.result.genre} *duração* \n${anu.result.durasi} *url* \n${anu.result.url}`
                   client.sendMessage(from, hasil, text, {quoted: mek,})
                   break
				case 'ytmp4':
					if (args.length < 1) return reply('cade o link po?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://st4rz.herokuapp.com/api/ytv2?url=${args[0]}`, {method: 'obter'})
					if (anu.error) return reply(anu.error)
				...
				case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envie a imagem com a legenda $ {prefix} adesivo ou resposta / tag de imagem`)
					}
					break
				case 'getses':
            	if (!isOwner) return reply(mess.only.ownerB)
            const sesPic = await client.getSnapshot()
            client.sendFile(from, sesPic, 'sessão.png', '^_^...', id)
            break	
				case 'gtts':	
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('o texto representa sei la')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('eu falhei desculpa :(')
			...
			case 'loli': 
				    try {
						...
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Lembrar! Citai Lolicon'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				case 'tiktokstalk':
					try {
						...
						console.log(`Error :`, color(e,'red'))
						reply('[ERRO] nome de usuário possivelmente inválido')
					}
					break
				case 'fitnah':	
				case 'fake':          
               if (!isGroup) return reply(mess.only.group)
				...
				if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				await client.client.leaveGroup(from, 'adioss', groupId)
                    break
				case 'bc': 
					if(!isOwner) return reply(' *quem é voce?') 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('mudou com sucesso o ícone do Grupo')
                break						
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (args[0] === 'Abrir') {
						...
						reply(`Grupo fechado pelo administrador`)
       ...
       client.sendMessage(from, 'Este é o número do meu proprietário> _ <, não envie spam ',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
           ...
           case 'demote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque quem vc quer tirar o poder!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `num mamou direito,perdeu adm :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`esse cara aq @${mentioned[0].split('@')[0]} nao mamou direito e perdeu adm `, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque quem vc quer dar adm!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `parabens ganhou ademir (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`esse aqui mamou direitim @${mentioned[0].split('@')[0]} e ganhou adm(+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque quem vc quer kitar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`flw @${mentioned[0].split('@')[0]} banido`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'clone':
					...
					try {
						...
						reply(' *eu falhei desculpa,tente de novo* ')
					}
					break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
